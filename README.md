...
- 内网低成本冷归档可使用“方案 A”：将飞牛 NAS 挂载到宿主机后，作为 Elasticsearch 文件系统快照仓库；详见 `DEPLOYMENT.md`

## 本地默认账号变更说明

> 旧本地演示账号已废弃，新登录账号是 `sys-superadmin`。

- 旧账号 `demo-admin`、`demo-operator`、`demo-viewer` 已不再作为本地登录账号创建
- 当前本地 bootstrap 超级管理员账号为 `sys-superadmin`
- 默认密码为 `Demo@2026`
- `system-automation` 是系统自动化保留账号，用于自动化操作归因，不作为日常人工管理账号
...
> 说明：飞牛 NAS / 通用 NAS 在当前仓库中推荐作为 Elasticsearch `snapshot repository` 使用，用于快照备份 / 冷归档；不建议直接作为 Elasticsearch `path.data` 热/温层数据盘。
