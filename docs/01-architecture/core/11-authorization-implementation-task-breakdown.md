...
| `data-services` | `RequireAuthenticatedIdentity` + `RequirePermission` 分别挂在 `query-api`、`audit-api`、`export-api` 的 `cmd/api/main.go` | `shared/auth` 已提供 typed capability/scope 与 `TenantReadScope`；`query-api` / `audit-api` 已切到显式 `TenantReadScope`，`export-api` 也已显式携带 actor scope、下沉 service capability 校验，并收口空租户转 `nil/match_none` 的旧语义，但仍保持 tenant-scoped | 继续补齐 export 侧 `owned vs tenant` scope 契约，并把剩余跨租户范围收口到 authorized tenant set |
...
| `services/data-services/export-api/internal/service/export_service.go` | 导出任务创建/读取/下载服务 | 已切到显式 actor `{tenant_id,user_id,tenant_read_scope,capabilities}`，并下沉 `export.job.create/read/download` service 校验；下一步补 `owned vs tenant` scope |
