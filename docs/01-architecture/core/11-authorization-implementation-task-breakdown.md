...
| `control-plane` | `RequireAuthenticatedIdentity`、`RequireOperatorRole`、`RequireAdminRole` 在 `services/control-plane/cmd/api/main.go` 统一接线 | `global_tenant_access.go` 已切到 capability + scope；`RequireAdminRole` / `RequireOperatorRole` 的 DB fallback 也已从角色名白名单切到权限/能力包匹配；`/api/v1/metrics/report` 已从 `auth_middleware.go` 的 path 特判迁到显式 `agentRoutes` | 继续把 route group 改成 capability guard，随后清理 `tenantScope=""` 和剩余 legacy fallback |
