太好了，给你 `V9__audit_verify_and_export.sql`（可直接进 Flyway）。
这版包含你要的三块：**trace 变更链视图、导出任务/WORM 清单、每日完整性校验**。
...
如果你愿意，我下一步给你 `V10__audit_partition_and_retention.sql`：
把 `data_change_audit` 做**按月分区 + 生命周期归档 + 冷热分层查询**，否则审计表量上去后性能会先成为瓶颈。