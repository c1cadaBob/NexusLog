非常好，直接给你 **V7 可执行脚本**。
目标：多租户按 `tenant_id` 隔离，按 `project_id` 可选二级隔离；并提供应用/审计/运维三类角色。

> 文件名建议：`V7__rls_multitenancy.sql`
...
如果你愿意，我下一步直接给你 **V8：审计不可篡改增强**（审计表 `append-only`、禁止 UPDATE/DELETE、审计触发器自动记录 before/after + trace_id）。