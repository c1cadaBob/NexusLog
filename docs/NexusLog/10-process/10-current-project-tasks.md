...
- [x] 8.1 打通 `ingest_pull_sources/ingest_pull_tasks` 落库并补充拉取配置（`agent_base_url/pull_interval_sec/pull_timeout_sec/key_ref`）
- [x] 8.2 新增并打通 `agent_pull_batches` 批次落库
- [x] 8.3 新增并打通 `agent_pull_cursors`、`agent_pull_auth_keys` 与 `ingest_dead_letters` 联动
- [x] 8.4 实施并验证 `batch_id + checksum` 幂等规则
- [x] 8.5 增加链路追踪字段（如 `request_id/task_id/batch_id`）
...
5. 已完成：新增执行器单测（成功/ES 失败）与 ingest 全量单测通过。
6. 已完成：真实环境联调证据补齐，成功任务 `task_id=7adb8307-effd-4528-8d87-8bb6ba6b8264`，链路状态为 `success -> package.acked -> receipt.ack -> cursor updated`。
7. 已完成：ES 实证查询命中 `6` 条文档（`logs-remote`，按 `task_id` 与 `batch_id` 均可命中），并完成环境恢复（容器版 `control-plane` 健康检查通过）。
8. 过程记录：联调中定位并修复两类环境问题（跨网络 DNS 与容器内 ES localhost 可达性），失败与修复证据均已归档。
