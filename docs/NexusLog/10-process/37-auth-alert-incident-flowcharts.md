...
│                   登录 / 会话建立流程（类图片样式）               │
├────────────────────────────────────────────────────────────────────┤
│ User → UI 登录页：输入账号密码并提交                               │
│ UI → Auth/API Service：POST /api/v1/auth/login                    │
│ Auth → PostgreSQL：查询用户 / 角色 / 状态                          │
│ PostgreSQL → Auth：返回用户与角色信息                              │
│   ├─ 校验成功：Auth → Session/JWT/Redis → 返回凭证                 │
│   │            Auth → UI：登录成功 + 会话信息                      │
│   │            UI：存储会话并跳转 Dashboard                        │
│   └─ 校验失败：Auth → UI：账号或密码错误                           │
│                UI：展示错误提示                                    │
└────────────────────────────────────────────────────────────────────┘
```
...
│ User → UI：访问受保护页面                                          │
│ UI → API：携带 token 发起请求                                      │
│ API：校验 token / session                                          │
│   ├─ 会话有效                                                      │
│   │    API → RBAC：解析租户 / 角色 / 菜单权限                      │
│   │    RBAC → PostgreSQL：查询权限映射                             │
│   │    PostgreSQL → RBAC：返回权限结果                             │
│   │      ├─ 有权限：返回页面数据 → UI 渲染                         │
│   │      └─ 无权限：返回 403 → UI 展示无权限提示                   │
│   └─ 会话无效：返回 401 → UI 清理会话并跳转登录页                  │
└────────────────────────────────────────────────────────────────────┘
```
...
│                  告警评估与通知闭环（类图片样式）                 │
├────────────────────────────────────────────────────────────────────┤
│ Scheduler 周期性触发 Evaluator                                     │
│   ↓ 读取启用规则                                                   │
│ Rule Repository 返回规则列表                                       │
│   ↓ Evaluator 按规则查询 ES                                        │
│ ES 返回命中数 / 聚合结果                                           │
│   ├─ 命中                                                          │
│   │    抑制判断 → 静默判断 → 创建 alert_event                      │
│   │    ├─ 非静默：发送通知 → 通知渠道回执                          │
│   │    └─ 静默期：跳过通知但保留事件                               │
│   │    ├─ critical：自动创建 Incident                              │
│   │    └─ 非 critical：仅保留告警事件                               │
│   └─ 未命中：结束本轮评估                                          │
└────────────────────────────────────────────────────────────────────┘
```
...
├────────────────────────────────────────────────────────────────────┤
│ [*]                                                                │
│  ↓ critical alert 自动创建 / 手动创建                              │
│ open                                                                │
│  ↓ ACK                                                              │
│ acked                                                               │
│  ↓ 开始调查                                                         │
│ investigating                                                       │
│  ├─ 已缓解 → mitigated → 验证完成 → resolved                       │
│  └─ 已确认解决 → resolved                                           │
