...
describe('SearchHistory delete flows', () => {
  ...
  it('reloads the filtered list when the backend reports the history row was already deleted', async () => {
    fetchQueryHistoryMock
      .mockResolvedValueOnce({
        items: [
    ...
    deleteQueryHistoryMock.mockResolvedValue(false);
  ...
  it('returns to the previous page after batch deleting rows that are already gone', async () => {
    const pageOneItems = Array.from({ length: 10 }, (_, index) =>
      buildHistoryItem(
        `history-missing-page-1-${index + 1}`,
        `missing-page-one-query-${index + 1}`,
        `2026-03-17T04:${String(index).padStart(2, '0')}:00.000Z`,
      ),
    );
    ...
    fetchQueryHistoryMock
      ...
      .mockResolvedValueOnce({
        items: [
          buildHistoryItem('history-missing-a', 'missing-last-page-query-a', '2026-03-17T03:58:00.000Z'),
          buildHistoryItem('history-missing-b', 'missing-last-page-query-b', '2026-03-17T03:57:00.000Z'),
        ],
        total: 12,
        page: 2,
        pageSize: 10,
    ...
    deleteQueryHistoryMock
      .mockRejectedValueOnce(Object.assign(new Error('query history not found'), { status: 404 }))
      .mockRejectedValueOnce(Object.assign(new Error('query history not found'), { status: 404 }));
    ...
    await waitFor(() => {
      expect(screen.getByText('missing-page-one-query-1')).toBeTruthy();
    });
    ...
    await waitFor(() => {
      expect(screen.getByText('missing-last-page-query-a')).toBeTruthy();
      expect(screen.getByText('missing-last-page-query-b')).toBeTruthy();
      expect(document.querySelector('.ant-pagination-item-active')?.textContent?.trim()).toBe('2');
    });

    fireEvent.click(screen.getByLabelText('选择查询历史 history-missing-a'));
    fireEvent.click(screen.getByLabelText('选择查询历史 history-missing-b'));
    ...
    await waitFor(() => {
      expect(deleteQueryHistoryMock).toHaveBeenCalledTimes(2);
      expect(deleteQueryHistoryMock).toHaveBeenNthCalledWith(1, 'history-missing-a');
      expect(deleteQueryHistoryMock).toHaveBeenNthCalledWith(2, 'history-missing-b');
    });
