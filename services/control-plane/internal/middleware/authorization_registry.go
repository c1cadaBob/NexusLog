...
func loadLegacyPermissionMappings(ctx context.Context, db *sql.DB) (map[string][]string, map[string][]string, bool, error) {
	if db == nil {
...
func loadUserAuthzEpoch(ctx context.Context, db *sql.DB, tenantID, userID string) (int64, error) {
	...
	var epoch int64
	if err := db.QueryRowContext(ctx, q, strings.TrimSpace(tenantID), strings.TrimSpace(userID)).Scan(&epoch); err != nil {
