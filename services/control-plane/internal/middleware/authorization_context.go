...
func emptyAuthorizationContextRecord() authorizationContextRecord {
	return authorizationContextRecord{
		Roles:       []string{},
		Permissions: []string{},
		Snapshot:    BuildAuthorizationContext("", []string{}, []string{}),
	}
}

func loadAuthorizationContext(ctx context.Context, db *sql.DB, tenantID, userID string) (authorizationContextRecord, error) {
	if db == nil {
		return emptyAuthorizationContextRecord(), nil
	}
