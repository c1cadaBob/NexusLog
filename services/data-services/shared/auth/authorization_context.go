...
func canBypassTenantScope(snapshot AuthorizationContextSnapshot) bool {
	if hasAuthorizationValue(snapshot.Capabilities, "*") {
		return true
	}
	if !hasAnyScope(snapshot.Scopes, "all_tenants", "system") {
		return false
	}
	return hasAnyCapability(snapshot.Capabilities,
		"log.query.read",
		"log.query.aggregate",
		"audit.log.read",
	)
...
func hasAnyCapability(capabilities []string, expected ...string) bool {
